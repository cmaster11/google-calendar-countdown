...
export default {
  methods: {
    ...
    async startLoginProcess() {
      ...
      ipcRenderer.on('google-oauth-reply', (event, {access_token, refresh_token}) => {
        this.$store.commit('logInUser', access_token, refresh_token);
        this.$store.dispatch('getUserData');
        this.$store.dispatch('saveCalendarList', access_token);
      });
      ipcRenderer.on('google-oauth-logout', (event) => {
